...
function toArray(e) {
  var tallValue = document.querySelector('#tall').value;
  var weightValue = document.querySelector('#weight').value;
  var bmiValue = (weightValue * (Math.pow(100, 2)) / Math.pow(tallValue, 2)).toFixed(2);
  ...
  //寫入結果圈圈
  var outcome = document.querySelector('.inner-circle p');
  ...
  var valueP = document.querySelector('.inner-circle p');
  var bmiSpan = document.querySelector('.inner-circle span');
  if (bmiValue < 18.5) {
...
//刪除紀錄
var btnRemoveAll = document.querySelector('.modalBtn');

function remove(e) {
  var dataNum = e.target.dataset.num;
  var dataNode = e.target.nodeName;
  console.log(dataNum);
  if (dataNode !== "BUTTON") {
    return;
  }
